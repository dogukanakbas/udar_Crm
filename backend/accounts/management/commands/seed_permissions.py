from django.core.management.base import BaseCommand
from accounts.models import Permission, RolePermission
from accounts.permissions_map import DEFAULT_ROLE_PERMS, PERMISSION_LABELS


class Command(BaseCommand):
    help = "Seed permissions and role-permission mappings"

    def handle(self, *args, **options):
        # ensure all permissions exist
        for code, label in PERMISSION_LABELS.items():
            Permission.objects.get_or_create(code=code, defaults={"description": label})

        # attach default role/group permissions only when that group has not been customized yet
        for role, perms in DEFAULT_ROLE_PERMS.items():
            if RolePermission.objects.filter(role__iexact=role).exists():
                continue
            for code in perms:
                perm = Permission.objects.get(code=code)
                RolePermission.objects.get_or_create(role=role, permission=perm)

        self.stdout.write(self.style.SUCCESS("Permissions seeded."))
