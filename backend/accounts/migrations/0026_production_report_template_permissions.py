...
ROLE_DEFAULTS = {
    'Admin': [code for code, _label in PERMISSIONS],
    'Manager': [code for code, _label in PERMISSIONS],
}


def seed_production_report_template_permissions(apps, schema_editor):
    ...
    group, _ = PermissionGroup.objects.get_or_create(
        group_id='production',
        defaults={'title': 'İmalat', 'display_order': 95},
    )
    permission_by_code = {}
    for order, (code, label) in enumerate(PERMISSIONS, start=180):
...
class Migration(migrations.Migration):
    dependencies = [('accounts', '0025_technical_drawing_permissions')]

    operations = [
        migrations.RunPython(seed_production_report_template_permissions, migrations.RunPython.noop),
    ]
