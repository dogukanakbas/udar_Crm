...
export function WarehouseOperationsPage() {
  ...
  const canAllocate = hasPermission(settings.role, permissions, 'warehouse_stock.allocate')
  const normalizedQuery = query.trim().toLocaleLowerCase('tr-TR')
  const unassignedProducts = products.filter((item: any) => {
    if (item.inventoryMode === 'warehouse' || Number(item.stock || 0) !== 0) return false
    if (categoryFilter !== 'all' && String(item.categoryId || '') !== categoryFilter) return false
    if (!normalizedQuery) return true
    return `${item.sku} ${item.name} ${item.categoryName || item.category || ''}`.toLocaleLowerCase('tr-TR').includes(normalizedQuery)
  })
  const productDefaultDetails = (product?: any) => {
    const details = Object.values(product?.attributeValues || {}).filter((value) => value != null && String(value).trim())
    return {
      detail_1_override: String(product?.templateDefaults?.primary || details[0] || ''),
      detail_2_override: String(product?.templateDefaults?.secondary || details[1] || ''),
    }
  }
  const openStockEntry = (product?: any) => {
    setSelected(null); setOperation('IN')
    setForm({ product_id: product?.id ? String(product.id) : '', location_id: '', quantity: '', note: '', reference: '', ...productDefaultDetails(product) })
  ...
  const submitOperation = async () => {
    ...
    try {
      ...
      setOperationOpen(false); await useAppStore.getState().hydrateFromApi({ force: true }); await loadStocks()
  ...
  return (
    <div className="space-y-4">
      <PageHeader title="Depo" description="Raf bazlı stokları arayın; giriş, çıkış, sayım ve transfer işlemlerini hesap yapmadan uygulayın."
        actions={<><input ref={importRef} type="file" accept=".xlsx" className="hidden" onChange={(event) => void importWorkbook(event.target.files?.[0])} /><Button onClick={() => openStockEntry()}><Plus className="mr-2 h-4 w-4" />Stok girişi</Button>{canAllocate && <Button variant="outline" onClick={() => { setAllocationProduct(null); setAllocations([blankAllocationRow()]); setAllocationOpen(true) }}><ArrowLeftRight className="mr-2 h-4 w-4" />Eski stok aktarımı</Button>}<Button variant="outline" onClick={() => importRef.current?.click()}><Upload className="mr-2 h-4 w-4" />İçe aktar</Button><Button variant="outline" onClick={() => void exportWorkbook()}><Download className="mr-2 h-4 w-4" />Dışa aktar</Button></>} />
      <Card>
      ...
      {warehouseFilter === 'all' && unassignedProducts.length > 0 && <Card><CardHeader><CardTitle>Henüz depoya alınmamış ürünler</CardTitle><CardDescription>Bu katalog ürünlerinin depo bakiyesi yoktur. İlgili ürünü seçerek ilk stok girişini doğrudan oluşturabilirsiniz.</CardDescription></CardHeader><CardContent className="space-y-2">{unassignedProducts.slice(0, 30).map((product: any) => <div key={product.id} className="flex flex-wrap items-center justify-between gap-3 rounded-md border border-border/70 px-3 py-2"><div><p className="text-sm font-medium">{product.sku} · {product.name}</p><p className="text-xs text-muted-foreground">{product.categoryName || product.category || 'Ürün grubu belirtilmemiş'} · Depo bakiyesi: 0</p></div><Button size="sm" variant="outline" onClick={() => openStockEntry(product)}><Plus className="mr-2 h-4 w-4" />İlk stok girişini yap</Button></div>)}{unassignedProducts.length > 30 && <p className="text-xs text-muted-foreground">İlk 30 kayıt gösteriliyor. Arama veya ürün grubu filtresiyle listeyi daraltın.</p>}</CardContent></Card>}
      ...
      {canAllocate && <Card><CardHeader><CardTitle>Eski stokları depolara aktar</CardTitle><CardDescription>Bu alan yalnız sistem geçişi içindir. Önceden tek toplam olarak tutulan ürün stoklarını ilk kez depo, raf ve varyantlara dağıtır. Günlük işlemler için stok girişi butonunu kullanın.</CardDescription></CardHeader><CardContent><div className="flex flex-wrap items-center justify-between gap-3 rounded-md border border-border/70 bg-muted/30 p-3"><div><p className="text-sm font-medium">{legacyProducts.length ? `${legacyProducts.length} ürün aktarım bekliyor` : 'Aktarım bekleyen eski stok bulunmuyor'}</p><p className="text-xs text-muted-foreground">Sıfır bakiyeli katalog ürünleri yukarıdaki listeden normal stok girişiyle depoya alınır.</p></div><Button variant="outline" onClick={() => { setAllocationProduct(null); setAllocations([blankAllocationRow()]); setAllocationOpen(true) }}><ArrowLeftRight className="mr-2 h-4 w-4" />Aktarım aracını aç</Button></div></CardContent></Card>}
